...
class Bot {
  ...
  attachListeners() {
    ...
    this.ircClient.on('names', (channel, nicks) => {
      if (this.namesRequestForVersion) {
        ...
        if (nickList.length) {
          ...
          this.sendToDiscord(null, channel, text);
        }
      }
    });
    ...
    this.ircClient.on('ctcp', (from, to, text, type, message) => {
      if (from && text && text.startsWith('VERSION ') && this.versionRequests[from]) {
        ...
        this.sendToDiscord(null, channel, textOut);
      }
    });
    ...
    this.ircClient.on('join', (channel, nick, message) => {
        ...
        this.sendToDiscord(null, channel, message);
    });
    ...
    this.ircClient.on('quit', (nick, reason, channels, message) => {
      ...
      message += `*`;
      channels.forEach((a) => {
        this.sendToDiscord(null, a, message);
      });
    });
